...
# Data Restore Script
# Restores data directory from backup created by data-backup.sh
...
usage() {
    ...
    echo " --source <path>     : Path to the data backup to restore from (e.g. /mnt/@data or snapshot)"
    echo " --dest <path>       : Destination path for restore (usually /data)"
    echo
    ...
    echo " • Full data directory restoration from backup"
    ...
    echo " # Preview data restore"
    echo " sudo $0 --source /mnt/@data --dest /data --dry-run"
    echo
    echo " # Perform data restore with snapshot"
    echo " sudo $0 --source /mnt/@data --dest /data --snapshots /mnt/@snapshots"
    echo
    echo " # Perform data restore without snapshot"
    echo " sudo $0 --source /mnt/@data --dest /data --no-snapshot"
    echo
    echo " # Show help"
    echo " $0 --help"
    exit 1
...
main() {
    ...
    # Print banner
    print_banner "Data Restore" "$BLUE"
    ...
    display_restore_details "data" "$RESTORE_SOURCE" "$RESTORE_DEST" "$snapshot_info"
    ...
    if [ "$DRY_RUN" = true ]; then
        ...
        # Execute dry-run
        rsync -aAXHv --numeric-ids --info=progress2 --dry-run "${RESTORE_SOURCE}/" "${RESTORE_DEST}/" | tee /tmp/data-restore-dry-run.txt
    ...
    echo -e "${BOLD}${RED}⚠ WARNING: This will restore data directory files and may overwrite current data!${NC}"
    echo -e "${YELLOW}A pre-restore snapshot will be created for rollback capability.${NC}"
    echo

    if ! confirm_execution "data restore" "n" "data" "$RESTORE_DEST" "${SNAPSHOT_PATH:-}"; then
        log_msg "INFO" "Data restore cancelled by user"
        exit 0
    fi

    # Execute restore with snapshot
    log_msg "INFO" "Starting data restore..."

    if execute_restore_with_snapshot "$RESTORE_SOURCE" "$RESTORE_DEST" "$SNAPSHOT_PATH" "data" "false"; then
        log_msg "SUCCESS" "Data restore completed successfully"
